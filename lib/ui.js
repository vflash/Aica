...
export function showStartupUI({ role, workdir, port, password, tunnelUrl, externalIP, publicUrl, useUPnP, useIP, useCloudflared, requestMode, autoMode }) {
    console.log();
    console.log(chalk.cyan.bold(` 🤖 aica — агент "${role}"`));
    ...
    console.log(`Используй aica-ассистента для работы с файлами проекта. Твоя роль(по умолчанию): ${role}.`);
    console.log(`Сервер: ${serverUrl}`);
    console.log(`Пароль: ${password} (запроси новый если устарел)`);
    console.log(`Используй доступные HTTP инструменты (code_interpreter,python,js и прочее) для запросов к серверу.`);
...
export function showPatchUI(id, action, recentRequests) {
    console.log();
    console.log(chalk.blue.bold(` 📨 ${action.action.toUpperCase()}`) + chalk.gray(` #${id}`));
...
export function showSequenceUI(id, action, recentRequests) {
    console.log();
    console.log(chalk.magenta.bold(` 📨 SEQUENCE`) + chalk.gray(` #${id}`));
    if (action.description) console.log(chalk.bold(action.description));
    if (action.reason) console.log(chalk.gray(` 💡 ${action.reason}`));
